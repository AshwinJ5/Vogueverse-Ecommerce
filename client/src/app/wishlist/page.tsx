                    ...
                    <main className="grow">
                        ...
                        <div className="flex flex-wrap justify-between gap-3 p-4 md:p-8">
                            <div className="flex min-w-72 flex-col gap-2">
                                <h1 className="text-text-light text-3xl sm:text-4xl font-black tracking-tighter">
                                    My Wishlist
                                </h1>
                                <p className="text-text-subtle-light text-sm sm:text-base font-normal">
                        ...
                        {wishlistItems.length > 0 ? (
                            /* ImageGrid - 2 columns on mobile */
                            <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 sm:gap-4 md:gap-6 p-4 md:p-8">
                                {wishlistItems.map((item) => (
                                    <div
                                        key={item.id}
                                        className="group relative flex flex-col gap-3 sm:gap-4 overflow-hidden rounded-lg sm:rounded-xl bg-card-light shadow-sm transition-shadow hover:shadow-lg border border-border-light"
                                    >
                                        {/* Remove Button - Smaller on mobile */}
                                        <div className="absolute top-2 right-2 z-10">
                                            <button
                                                onClick={() => removeFromWishlist(item.id)}
                                                className="flex h-7 w-7 sm:h-9 sm:w-9 items-center justify-center rounded-full bg-card-light/80 backdrop-blur-sm text-accent hover:text-primary transition-colors shadow-sm"
                                        ...
                                        <div className="flex flex-col gap-2 sm:gap-4 p-3 sm:p-4 pt-0">
                                            <div className="min-h-12 sm:min-h-14">
                                                <p className="text-text-light text-sm sm:text-base font-semibold leading-tight line-clamp-2">
                                                    {item.name}
                                                </p>
                                                <p className="text-text-subtle-light text-xs sm:text-sm font-normal leading-normal mt-1">
                                                    {item.price}
                                                </p>
                                            </div>
                                            <button className="flex w-full cursor-pointer items-center justify-center overflow-hidden rounded-lg h-8 sm:h-10 px-3 sm:px-4 bg-primary text-white text-xs sm:text-sm font-bold tracking-wide transition-opacity hover:opacity-90 hover:shadow-md">
